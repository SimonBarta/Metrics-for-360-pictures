...
aveY1 = convn(Y1, aveKernel,'same');
aveY2 = convn(Y2, aveKernel,'same');
Y1 = aveY1(1:Down_step:end,1:Down_step:end);
Y2 = aveY2(1:Down_step:end,1:Down_step:end);

IxY1 = convn(Y1, dx, 'same');     
IyY1 = convn(Y1, dy, 'same');    
gradientMap1 = sqrt(IxY1.^2 + IyY1.^2);

IxY2 = convn(Y2, dx, 'same');     
IyY2 = convn(Y2, dy, 'same');
